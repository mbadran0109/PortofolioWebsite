...
document.addEventListener('DOMContentLoaded', function () {
  const fadeEls = Array.from(document.querySelectorAll('.animate-fade-up'));
  fadeEls.forEach(el => {
  ...
  toggleButtons.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (isOpen) {
        ...
        body.offsetHeight; // Force DOM reflow to ensure the transition plays smoothly
  ...
  async function ensureEmailJS() {
    ...
    if (!window.emailjs) {
      throw new Error('EmailJS loaded but global not found');
    }
  ...
  contactForm.addEventListener('submit', async (ev) => {
    ...
    const templateParams = {
      ...
      message: message,
      title: `Website inquiry: ${selectedService || 'General'} - ${first} ${last}`,
      time: new Date().toLocaleString()
    };

    const success = document.getElementById('contact-success');
    if (success) {
      // Display the "Preparing email..." loading state before the network request
